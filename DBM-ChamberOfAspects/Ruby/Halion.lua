...
function mod:SPELL_CAST_SUCCESS(args)--We use spell cast success for debuff timers in case it gets resisted by a player we still get CD timer for next one
	if args.spellId == 74792 then
		if self:IsHeroic() then
			timerShadowConsumptionCD:Start(30)
		else
...
function mod:OnSync(msg, target)
	...
	elseif msg == "Phase2" and self.vb.phase < 2 then
		...
		timerShadowConsumptionCD:Start(30)--Edited. not exact, 15 seconds from tank aggro, but easier to add 5 seconds to it as a estimate timer than trying to detect this
	...
	elseif msg == "Phase3" and self.vb.phase < 3 then
		self:SetStage(3)
		warnPhase3:Show()
		timerMeteorCD:Start(33) --These i'm not sure if they start regardless of drake aggro, or if it varies as well.
