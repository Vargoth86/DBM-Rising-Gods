...
function mod:OnCombatStart(delay)
	...
	if self:IsHeroic() then
		...
		timerRisingAnger:Start(15-delay)
	else
...
function mod:WormsEmerge()
	...
	if not self.vb.AcidmawDead then
		if self.vb.DreadscaleActive then
			timerSweepCD:Start(16)			-- Log review: 22-24s (N/H?)
			timerParalyticSprayCD:Start(20)	-- Log review: 18-20s (N/H?)
		else
	...
	self:ScheduleMethod(33, "WormsSubmerge")
end
